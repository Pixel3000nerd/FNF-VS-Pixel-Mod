...
class PlayState extends MusicBeatState
{
	...
	// VS Pixel stuff
	var vspixel_glitch:FlxSprite;
	var whiteFlash:FlxSprite;
	var calledStep:Int = 0;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'march-two-seven':
				dialogue = CoolUtil.coolTextFile(Paths.txt('march-two-seven/march-two-sevenDialogue'));
			case 'quainty-four':
				dialogue = CoolUtil.coolTextFile(Paths.txt('quainty-four/quainty-fourDialogue'));
			case 'desk-rattle-moment':
				dialogue = CoolUtil.coolTextFile(Paths.txt('desk-rattle-moment/desk-rattle-momentDialogue'));
		}
			...
			case 'march-two-seven':
				{
					...
					var bg:FlxSprite = new FlxSprite(-38, -298).loadGraphic(Paths.image('redmugday-sky'));
					bg.scrollFactor.set(0.5, 0.5);
					bg.scale.set(1, 1);
					add(bg);

					var hood:FlxSprite = new FlxSprite(-500, -410).loadGraphic(Paths.image('redmugday-hood'));
					add(hood);
					hood.scale.set(0.70, 0.70);

					var street:FlxSprite = new FlxSprite(-500, -410).loadGraphic(Paths.image('redmugday-foreground'));
					add(street);
					street.scale.set(0.70, 0.70);
				}
			case 'quainty-four':
				{
					curStage = 'redmug-tarde';

					var bg:FlxSprite = new FlxSprite(-38, -298).loadGraphic(Paths.image('redmugtarde-sky'));
					bg.scrollFactor.set(0.5, 0.5);
					bg.scale.set(1, 1);
					add(bg);

					var hood:FlxSprite = new FlxSprite(-500, -410).loadGraphic(Paths.image('redmugtarde-hood'));
					add(hood);
					hood.scale.set(0.70, 0.70);

					var street:FlxSprite = new FlxSprite(-500, -410).loadGraphic(Paths.image('redmugtarde-foreground'));
					add(street);
					street.scale.set(0.70, 0.70);
				}
			case 'desk-rattle-moment':
				{
					curStage = 'redmug-night';

					var bg:FlxSprite = new FlxSprite(-38, -298).loadGraphic(Paths.image('redmugnight-sky'));
					bg.scrollFactor.set(0.5, 0.5);
					bg.scale.set(1, 1);
					add(bg);

					var hood:FlxSprite = new FlxSprite(-500, -410).loadGraphic(Paths.image('redmugnight-hood'));
					hood.scrollFactor.set(0.7, 0.7);
					hood.scale.set(0.70, 0.70);
					add(hood);

					var street:FlxSprite = new FlxSprite(-500, -410).loadGraphic(Paths.image('redmugnight-foreground'));
					street.scale.set(0.70, 0.70);
					add(street);

					// load animated glitch texture
					vspixel_glitch = new FlxSprite(-500, -410).loadGraphic(Paths.image('vspixel-glitch'), true, 2560, 1600);
					vspixel_glitch.animation.add("pixelGlitch", [0, 1, 2, 3, 4, 5, 6, 7], 8);
					// set opacity to 0 for fade transition
					vspixel_glitch.alpha = 0;
					vspixel_glitch.scale.set(0.70, 0.70);
					add(vspixel_glitch);
					vspixel_glitch.animation.play("pixelGlitch");

					whiteFlash = new FlxSprite(-500, -410).makeGraphic(2500, 1600, FlxColor.WHITE);
					whiteFlash.alpha = 0;
					add(whiteFlash);
				}
			default:
		...
		if (curStage == 'redmug-night')
		{
			FlxG.camera.follow(camFollow, LOCKON, 0.008 * (30 / (cast(Lib.current.getChildAt(0), Main)).getFPS()));
		}
		else
		{
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				...
				case 'redmug-day' | 'redmug-tarde' | 'redmug-night':
					babyArrow.frames = Paths.getSparrowAtlas('vsPixel-NOTE_assets');
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			...
			case 'redmug-night':
				if (curBeat >= 3 && vspixel_glitch.alpha < 1)
				{
					vspixel_glitch.alpha += 0.005;
				}
				if (curStep != calledStep)
				{
					switch (curStep)
					{
						case 80:
							FlxG.camera.follow(camFollow, LOCKON, 0.04 * (30 / (cast(Lib.current.getChildAt(0), Main)).getFPS()));
							FlxG.camera.shake(.025, .12);
							whiteFlash.alpha = .5;
							new FlxTimer().start(.033, function(tmr:FlxTimer)
							{
								remove(whiteFlash);
							});
					}
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		if (daRating != 'shit' || daRating != 'bad')
		{
			...
			// Quaint
			if (curStage == "redmug-tarde" && daRating == "sick")
			{
				rating.loadGraphic(Paths.image("quaint"));
			}
			else
			{
